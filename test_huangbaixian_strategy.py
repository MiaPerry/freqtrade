...
def simple_huangbaixian_strategy(dataframe):
    """
    简单的黄白线策略：
    - 当短期均线上穿长期均线时（金叉）买入
    - 当短期均线下穿长期均线时（死叉）卖出
...
def backtest_strategy(dataframe, initial_capital=10000):
    ...
    buy_signals, sell_signals = simple_huangbaixian_strategy(dataframe)
...
def main():
    ...
    for pair in available_pairs:
        ...
        if df is not None and pair in results:
            ...
            strategy_return = results[pair]['total_return']
            
            print(f"\n{pair}:")
            print(f"  策略回报: {strategy_return:.2f}%")
            print(f"  买入持有回报: {buy_hold_return:.2f}%")
            print(f"  差异: {strategy_return - buy_hold_return:+.2f}%")
